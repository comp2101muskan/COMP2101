...
function error-exit {
	echo "ERROR: Stopping Script $@" >&2
	exit "${2-1}"
}
#This function displays help information if the user asks for it on the command line or gives us a bad command line
function displayhelp {
	echo " Syntax: sysconfig.sh [--host|domain|ipconfig|os|cpu|memory|disk|printer]"
	echo " --host      :It is used to show host information."
	echo " --domain    :It is used NIS domain of host."
	echo " --ipconfig  :It is used for displaying current IP and Netork Configuration."
	echo " --os        :It is used to identify the OS."
	echo " --cpu       :It displays information about the CPU"
	echo " --memory    :It displays the memory usage"
	echo " --disk      :It displays the remaining disk space"
	echo " --printer   :It displays the list of printers connected to your device"
}
...
function cleanup {
	rm /tmp/sysreport.$$
	echo "Removing temporary files generated during script"
	exit 1
}
